...
document.addEventListener('DOMContentLoaded', function() {
   ...
   // **캘린더에 추가할 이벤트 생성 함수**
 function createCalendarEvents(comparisonResult) {
    const events = [];
   
  
    // 비교 결과에서 필요한 정보 추출하여 이벤트 생성
    comparisonResult.matches.forEach(match => {
     // 날짜 정보 가져오기 (임시: 현재 날짜 사용, 실제 구현에서는 문서 날짜 사용)
     const eventDate = new Date().toISOString().split('T')[0];
   
  
     // 일정 제목 생성
     const eventTitle = `${comparisonResult.document_types.doc1} vs ${comparisonResult.document_types.doc2} - ${match.product_name}`;
     ...
     events.push({
      title: `${eventTitle} (시작)`,
      date: eventDate,
      category: 'shipping', // 또는 'business' 등 적절한 카테고리
      description: `
       품목: ${match.product_name}
       수량: ${match.quantity}
       가격: ${match.price}
      `,
      // 필요한 경우 PersonalEvent 모델에 있는 필드 추가
      // 예: start_time, end_time, all_day 등
     });
...
function addEventsToCalendar(events, doc1Id, doc2Id) {
    ...
    // 엔드포인트 조정: calendar/event 사용
    fetch(`${API_BASE_URL}/calendar/events`, {  // event_id가 필요하지만 새 이벤트이므로 임의의 값 사용
        method: 'POST',  // 이 메소드는 routes.py에서 지원하지 않음
        headers: {
    ...
    function showComparisonResult(result, doc1Id, doc2Id) {
        ...
        if (addToCalendarBtn) {
            addToCalendarBtn.addEventListener('click', () => {
                // 비교 결과에서 일정 생성
                const eventsToAdd = createCalendarEvents(result);
                // 캘린더에 일정 추가 API 호출
