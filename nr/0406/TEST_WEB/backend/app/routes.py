...
def calendar_events():
    ...
    if request.method == 'GET':
        try:
            ...
            # 이벤트 쿼리 초기화 - manually_compared 필터 추가
            shipping_events_query = DocumentComparison.query.filter(
                DocumentComparison.manually_compared == True
            )
            ...
            if start_date and end_date:
                try:
                    ...
                    # 선적 이벤트 쿼리에 날짜 필터 추가
                    shipping_events_query = shipping_events_query.filter(
                        DocumentComparison.comparison_date.between(start, end)
                    )
                except ValueError:
                    pass
            
            # 수동 비교된 문서 조회
            manually_compared_docs = shipping_events_query.all()
            
            # 이벤트 생성
            shipping_events = []
            
            for doc_comparison in shipping_events_query:
                try:
                    # 비교 데이터 파싱 (예외 처리 추가)
                    if not doc_comparison.comparison_data:
                        continue
                        
                    comparison_data = json.loads(doc_comparison.comparison_data)
                    
                    # 문서 정보 가져오기 (예외 처리 추가)
                    doc1 = None
                    if doc_comparison.document1_type == 'invoice':
                        doc1 = Invoice.query.get(doc_comparison.document1_id)
                    else:
                        doc1 = Order.query.get(doc_comparison.document1_id)
                    
                    if not doc1:
                        continue
                    
                    # 날짜 정보 준비
                    event_date = doc1.created_at.strftime('%Y-%m-%d')
                    
                    # 일치 항목 처리
                    for match in comparison_data.get('matches', []):
                        if not match.get('product_name'):
                            continue
                            
                        # 시작 이벤트
                        start_event = {
                            'id': f"comparison_{doc_comparison.id}_{match.get('product_key', 'unknown')}_start",
                            'title': f"{doc_comparison.brand} - {match.get('product_name', '제품')} (시작)",
                            'start': event_date,
                            'date': event_date,  # calendar.js에서 사용하는 필드
                            'type': 'shipping',
                            'category': 'shipping',  # calendar.js에서 사용하는 필드
                            'color': '#4CAF50',
                            'brand': doc_comparison.brand,
                            'season': doc_comparison.season,
                            'document_id': doc_comparison.id,
                            'match_percentage': doc_comparison.match_percentage,
                            'schedule_type': 'start',
                            'is_confirmed': True,
                            'description': f"품목: {match.get('product_name')}\n수량: {match.get('quantity', '정보 없음')}\n가격: {match.get('price', '정보 없음')}"
                        }
                        
                        # 완료 이벤트
                        end_event = {
                            'id': f"comparison_{doc_comparison.id}_{match.get('product_key', 'unknown')}_end",
                            'title': f"{doc_comparison.brand} - {match.get('product_name', '제품')} (완료)",
                            'start': event_date,
                            'date': event_date,  # calendar.js에서 사용하는 필드
                            'type': 'shipping',
                            'category': 'shipping',  # calendar.js에서 사용하는 필드
                        ...
                        shipping_events.append(start_event)
                        shipping_events.append(end_event)
                except Exception as inner_e:
                    # 개별 문서 처리 중 오류 발생 시 계속 진행
                    print(f"문서 ID {doc_comparison.id} 처리 중 오류: {str(inner_e)}")
                    continue
