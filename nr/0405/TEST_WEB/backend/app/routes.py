    ...
    try:
        ...
        try:
            ...
            new_comparison = DocumentComparison(
                ...
                brand=brand,
                season=season,
                manually_compared=True  # 수동 비교 플래그 추가
            )
...
def get_calendar_events():
   ...
   try:
       ...
       # 이벤트 생성
       shipping_events = []
       for doc_comparison in manually_compared_docs:
           # 비교 데이터 파싱
           ...
           for match in comparison_data.get('matches', []):
               event = {
                   'id': f"comparison_{doc_comparison.id}_{match['product_key']}",
                   'title': f"{doc_comparison.brand} - {match.get('product_name', '제품')}",
                   'start': doc1.created_at.strftime('%Y-%m-%d'),
                   'type': 'shipping',
                   'color': '#4CAF50',  # 성공적인 비교를 나타내는 색상
