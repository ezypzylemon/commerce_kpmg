...
class WordFrequencyAnalyzer:
    def __init__(self):
        ...
        self.monthly_freq = None
        self.category_freq = None
        self.categories = {
    ...
    def get_top_words(self, n=30):
        """상위 n개 단어와 빈도수를 반환합니다."""
        if not self.word_counts:
            raise ValueError("단어 빈도가 계산되지 않았습니다. analyze() 메서드를 먼저 호출하세요.")
        
        top_words = self.word_counts.most_common(n)
        return pd.DataFrame(top_words, columns=['word', 'frequency'])
    
    def analyze_monthly_trends(self, texts, dates, keywords):
        """월별 키워드 빈도 추이를 분석합니다."""
        if not keywords:
            return None
        ...
        # 월별 키워드 빈도 계산
        keyword_monthly_freq = {keyword: [] for keyword in keywords}
        months = sorted(df['year_month'].unique())
        
        for month in months:
            month_texts = df[df['year_month'] == month]['text']
            month_tokens = []
            for tokens in month_texts:
    ...
    def analyze_category_frequencies(self, texts, category):
        """특정 카테고리의 키워드 빈도를 분석합니다."""
        if category not in self.categories:
            raise ValueError(f"유효하지 않은 카테고리입니다: {category}")
        
        category_words = self.categories[category]
        category_counts = {word: sum(1 for tokens in texts if word in tokens) for word in category_words}
        
        self.category_freq = pd.DataFrame({
            'keyword': list(category_counts.keys()),
            'frequency': list(category_counts.values())
