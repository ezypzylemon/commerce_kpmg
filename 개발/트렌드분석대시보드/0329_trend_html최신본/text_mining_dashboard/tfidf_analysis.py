...
class TfidfAnalyzer:
    ...
    def get_top_keywords(self, n=30):
        """상위 n개의 TF-IDF 키워드를 반환합니다."""
        if not self.tfidf_matrix:
            raise ValueError("TF-IDF 행렬이 계산되지 않았습니다. fit() 메서드를 먼저 호출하세요.")
        
        # 코퍼스 전체에서의 단어별 TF-IDF 평균 점수 계산
        mean_tfidf = np.array(self.tfidf_matrix.mean(axis=0)).flatten()
        word_scores = [(self.feature_names[i], mean_tfidf[i]) for i in range(len(self.feature_names))]
        word_scores.sort(key=lambda x: x[1], reverse=True)
        
        return pd.DataFrame(word_scores[:n], columns=['word', 'tfidf_score'])
    
    def find_similar_documents(self, doc_id=None, query=None, n=5, threshold=0.3):
        """유사 문서를 검색합니다."""
        if not self.cosine_sim:
            raise ValueError("문서 간 유사도가 계산되지 않았습니다. fit() 메서드를 먼저 호출하세요.")
        
        if doc_id is not None:
            # 문서 ID로 검색
            sim_scores = list(enumerate(self.cosine_sim[doc_id]))
            sim_scores = [(i, score) for i, score in sim_scores 
                         if i != doc_id and score >= threshold]
        elif query is not None:
            # 키워드로 검색
            query_vector = self.vectorizer.transform([query])
            doc_sim = cosine_similarity(query_vector, self.tfidf_matrix).flatten()
            sim_scores = list(enumerate(doc_sim))
            sim_scores = [(i, score) for i, score in sim_scores if score >= threshold]
        else:
            raise ValueError("doc_id 또는 query 중 하나를 지정해야 합니다.")
    ...
    def cluster_documents(self, n_clusters=5):
        ...
        # 차원 축소
        svd = TruncatedSVD(n_components=min(100, self.tfidf_matrix.shape[1]-1))
        ...
        return clusters, reduced_matrix
