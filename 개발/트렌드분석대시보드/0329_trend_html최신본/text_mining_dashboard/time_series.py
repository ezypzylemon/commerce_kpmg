...
class TimeSeriesAnalyzer:
    ...
    def analyze_time_series(self, df):
        ...
        # 일별 기사 수 분석
        daily_counts = df.groupby(df['upload_date'].dt.date).size().reset_index(name='article_count')
    ...
    def _analyze_keyword_trends(self, df):
        """키워드 트렌드를 분석합니다."""
        # 모든 토큰 수집
        all_tokens = []
        ...
        # 키워드별 월간 트렌드 분석
        keyword_trends = {}
        for keyword in default_keywords[:5]:
            df[f'has_{keyword}'] = df['token_list'].apply(lambda tokens: 1 if keyword in tokens else 0)
            trend = df.groupby('year_month')[f'has_{keyword}'].sum().reset_index()
            trend.columns = ['year_month', 'frequency']
